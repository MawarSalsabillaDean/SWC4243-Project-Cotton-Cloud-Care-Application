...
public class PapulePageController implements Initializable {
    @FXML
    ...
    @FXML
    private Label label_howForm_papule;

    @FXML
    private Label label_process_papule;

    @FXML
    private Text text_explanation_papule;

    @FXML
    private VBox vbox_factors;

    @FXML
    private ImageView image_factors_papule;

    @FXML
    private Label label_factors_papule;
    ...
    @FXML
    private ImageView image_prevention_papule;

    @FXML
    private Label label_prevention_papule;

    @FXML
    private Text text_prevention;

    @FXML
    private VBox vbox_prevention_papule;
    ...
    private void goBack() {
        try {
            // Load the FXML file for the AcneTypePage and create a new scene.
            Parent AcneTypePage = FXMLLoader.load(getClass().getResource("AcneTypePage.fxml"));
            Scene AcneTypePageScene = new Scene(AcneTypePage);

            // Get the current window (stage) from the back button's scene.
            Stage window = (Stage) image_backbutton.getScene().getWindow();

            // Change the scene displayed by the stage to the AcneTypePage.
    ...
    private void goHome() {
        try {
            // Load the FXML file for the MenuPage and create a new scene.
            Parent MenuPage = FXMLLoader.load(getClass().getResource("MenuPage.fxml"));
            Scene MenuScene = new Scene(MenuPage);

            // Use the same stage as before to change the scene to MenuPage.
            Stage window = (Stage) image_homeButton.getScene().getWindow();

            // Set the new scene on the stage to display the home page.
